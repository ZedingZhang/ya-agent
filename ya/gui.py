...
TEXT = {
    "en": {
        ...
        "status_ready": "Ready", "status_working": "Ya is working...", "status_done": "Done",
    ...
    "zh-CN": {
        ...
        "auto": "自动", "on": "开启", "off": "关闭", "status_ready": "就绪", "status_working": "Ya 正在处理...", "status_done": "完成",
...
class YaApp(ttk.Frame):
    ...
    def _build_ask(self) -> None:
        ...
        ttk.Combobox(controls, textvariable=self.model_var, values=("flash", "pro"), state="readonly", width=8).grid(row=0, column=1, padx=(4, 12))
        ...
        ttk.Checkbutton(controls, text=self.t("toa"), variable=self.toa_var, command=self._toggle_toa).grid(row=0, column=5)
        ...
        ttk.Checkbutton(controls, text=self.t("stream"), variable=self.stream_var).grid(row=1, column=0, columnspan=4, sticky="w", pady=(6, 0))
        self.send_button = ttk.Button(controls, text=self.t("send"), command=self._ask)
        self.send_button.grid(row=1, column=5, columnspan=2, sticky="e", pady=(6, 0))
    ...
    def _ask(self) -> None:
        ...
        if not task or self.running:
            return
        options = GuiTaskOptions(task, self._web_mode(), self.toa_var.get(), int(self.workers_var.get()), self.stream_var.get())
    ...
    def _run_worker(self, options: GuiTaskOptions) -> None:
        ...
        try:
            result = self.controller.run(options, on_content=on_content)
