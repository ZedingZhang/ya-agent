...
原生 GUI 在同一个工作区工作台中处理普通问答和本地任务。选择文件夹后，必须显式开启 **启用本地工具** 才会
向 Agent 授权本地读取；Ya 会在本地记住上次的文件夹路径，但每次启动时本地模式都默认关闭。文件树仅用于浏览
结构，不会把文件内容发送给模型；Agent 的列目录、读取和搜索活动会在右栏显示，但不会展示文件内容。每项变更
